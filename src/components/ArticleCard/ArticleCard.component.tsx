...
export const ArticleCard = ({ blog, clientEmail, index, noShadow }: ArticleCardTypes) => {
	...
	const relTime = useMemo(() => {
		...
		return /^[a-zA-Z\s]+$/.test(result) ? result.charAt(0).toUpperCase() + result.slice(1) : result;
	}, [updatedAt]);
							...
							<BlogAuthor>
								...
								{clientEmail === authorEmail && (
									<ButtonsContainer>
										...
										<DeleteButton onClick={showDeleteModalHandler}>Delete</DeleteButton>
									</ButtonsContainer>
								)}
								<Modal showModal={showDeleteModal} hideModal={() => setShowDeleteModal(false)}>
									<DeleteCard
										blogTitle={blogTitle}
